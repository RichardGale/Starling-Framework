...
namespace events {
    ...
    {
        private:  std::map<std::string, void*> mEventListeners;

        /** Helper object. */
        private: static  std::vector<void*> sBubbleChains;

        /** Creates an EventDispatcher. */
        public:          EventDispatcher();

        /** Registers an event listener at a certain object. */
        public: void     addEventListener(std::string type, Function* listener);

        /** Removes an event listener from the object. */// check for duplicates
        public: void     removeEventListener(std::string type, Function* listener);

        /** Removes all event listeners with a certain type, or all of them if type is null. 
         *  Be careful when removing all event listeners: you never know who else was listening. */
        public: void     removeEventListeners(std::string type=NULL);
         ...
         *  avoid allocations. */
        public: void     dispatchEventWith(std::string type, bool bubbles   =false, Object* data=NULL);

        /** Returns if there are listeners registered for a certain event type. */
        public: bool     hasEventListener(std::string type);
    };
}
}
