...
namespace flash { namespace display { class Loader; } }
namespace flash { namespace events { class Event; } }
namespace flash { namespace events { class IOErrorEvent; } }
namespace flash { namespace events { class ProgressEvent; } }
namespace flash { namespace media { class Sound; } }
namespace flash { namespace media { class SoundChannel; } }
namespace flash { namespace media { class SoundTransform; } }
namespace flash { namespace net { class FileReference; } }
namespace flash { namespace net { class URLLoader; } }
namespace flash { namespace net { class URLLoaderDataFormat; } }
namespace flash { namespace net { class URLRequest; } }
namespace flash { namespace system { class ImageDecodingPolicy; } }
namespace flash { namespace system { class LoaderContext; } }
namespace flash { namespace utils { class ByteArray; } }
namespace flash { namespace utils { class Dictionary; } }
namespace flash { namespace utils { class clearTimeout; } }
namespace flash { namespace utils { class describeType; } }
...
namespace utils {
    class AssetManager
    {
        private: const std::vector<std::string>* SUPPORTED_EXTENSIONS;

        private:  float mScaleFactor;
        private:  bool mUseMipMaps;
        private:  bool mCheckPolicyFile;
        private:  bool mVerbose;

        private:  std::vector<void*> mRawAssets;
        private:  std::map<std::string, void*> mTextures;
        private:  std::map<std::string, void*> mAtlases;
        private:  std::map<std::string, void*> mSounds;

        /** helper objects */
        private:  std::vector<std::string>* sNames;

        /** Create a new AssetManager. The 'scaleFactor' and 'useMipmaps' parameters define
         *  how enqueued bitmaps will be converted to textures. */
        public:          AssetManager(float scaleFactor =1, bool useMipmaps   =false);
         ...
         *  texture atlases. */
        public: Texture* getTexture(std::string name);

        /** Returns all textures that start with a certain string, sorted alphabetically
         *  (especially useful for "MovieClip"). */
        public: std::vector<Texture*>* getTextures(std::string prefix="", std::vector<Texture*>* result=NULL);

        /** Returns all texture names that start with a certain string, sorted alphabetically. */
        public: std::vector<std::string>* getTextureNames(std::string prefix="", std::vector<std::string>* result=NULL);

        /** Returns a texture atlas with a certain name, or null if it's not found. */
        public: TextureAtlas* getTextureAtlas(std::string name);

        /** Returns a sound with a certain name. */
        public: Sound*   getSound(std::string name);

        /** Returns all sound names that start with a certain string, sorted alphabetically. */
        public: std::vector<std::string>* getSoundNames(std::string prefix="");

        /** Generates a new SoundChannel object to play back the sound. This method returns a 
         *  SoundChannel object, which you can access to stop the sound and to control volume. */
        public: SoundChannel* playSound(std::string name, float startTime =0, int loops=0,
                                  SoundTransform* transform=NULL);

        // direct adding

        /** Register a texture under a certain name. It will be available right away. */
        public: void     addTexture(std::string name, Texture* texture);

        /** Register a texture atlas under a certain name. It will be available right away. */
        public: void     addTextureAtlas(std::string name, TextureAtlas* atlas);

        /** Register a sound under a certain name. It will be available right away. */
        public: void     addSound(std::string name, Sound* sound);

        // removing

        /** Removes a certain texture, optionally disposing it. */
        public: void     removeTexture(std::string name, bool dispose   =true);

        /** Removes a certain texture atlas, optionally disposing it. */
        public: void     removeTextureAtlas(std::string name, bool dispose   =true);

        /** Removes a certain sound. */
        public: void     removeSound(std::string name);
        ...
        public: std::string enqueueWithName(Object* asset, std::string name=NULL);
         ...
         */
        public: void     loadQueue(Function* onProgress);

        private: void     loadRawAsset(std::string name, Object* rawAsset, std::vector<XML*>* xmls,
        ...
        protected: void     addBitmapTexture(std::string name, Bitmap* bitmap);

        // properties

        /** When activated, the class will trace information about added/enqueued assets. */
        public: bool         verbose();
        public: void         verbose(bool value);

        /** For bitmap textures, this flag indicates if mip maps should be generated when they 
         *  are loaded; for ATF textures, it indicates if mip maps are valid and should be
         *  used. */
        public: bool         useMipMaps();
        public: void         useMipMaps(bool value);
        ...
        public: void         checkPolicyFile(bool value);
    };
}
}
