...
#include "starling/core/TouchMarker.h"
...
namespace core {
        ...
        void TouchProcessor::advanceTime(float passedTime)
        {
            ...
            while (mQueue.size() > 0)
            {
                ...
                for (i=mCurrentTouches.size()-1; i>=0; --i)
        ...
        }   // On OS X, we get mouse events from outside the stage; on Windows, we do not.// This method enqueues an artifial hover point that is just outside the stage.// That way, objects listening for HOVERs over them will get notified everywhere.
