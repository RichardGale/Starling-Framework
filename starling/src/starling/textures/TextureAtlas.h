...
namespace textures {
    class TextureAtlas
    {
        ...
        private:  std::map<std::string, void*> mTextureRegions;
        private:  std::map<std::string, void*> mTextureFrames;

        /** helper objects */
        private:  std::vector<std::string>* sNames;

        /** Create a texture atlas from a texture by parsing the regions from an XML file. */
        public:          TextureAtlas(Texture* texture, XML* atlasXml=NULL);
        ...
        protected: void     parseAtlasXml(XML* atlasXml);
        ...
        public: std::vector<std::string>* getNames(std::string prefix="", std::vector<std::string>* result=NULL);

        /** Returns the region rectangle associated with a specific name. */
        public: Rectangle* getRegion(std::string name);

        /** Returns the frame rectangle of a specific region, or <code>null</code> if that region 
         *  has no frame. */
        public: Rectangle* getFrame(std::string name);

        /** Adds a named region for a subtexture (described by rectangle with coordinates in 
         *  pixels) with an optional frame. */
        public: void     addRegion(std::string name, Rectangle* region, Rectangle* frame=NULL);

        /** Removes a region with a certain name. */
        public: void     removeRegion(std::string name);
