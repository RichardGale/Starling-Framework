...
namespace flash { namespace display3D { class Context3DProgramType; } }
namespace flash { namespace display3D { class Context3DVertexBufferFormat; } }
namespace flash { namespace display3D { class IndexBuffer3D; } }
namespace flash { namespace display3D { class Program3D; } }
namespace flash { namespace display3D { class VertexBuffer3D; } }
...
namespace starling { namespace display { class QuadBatch; } }
namespace starling { namespace display { class Stage; } }
namespace starling { namespace errors { class AbstractClassError; } }
namespace starling { namespace errors { class MissingContextError; } }
...
namespace filters {
    class FragmentFilter
    {
        ...
        protected: const bool PMA;

        /** The standard vertex shader code. It will be used automatically if you don't create
         *  a custom vertex shader yourself. */
        protected: const std::string STD_VERTEX_SHADER;  // pass texture coordinates to fragment program

        /** The standard fragment shader code. It just forwards the texture color to the output. */
        protected: const std::string STD_FRAGMENT_SHADER; // just forward texture color

        private:  int mVertexPosAtID;
        private:  int mTexCoordsAtID;
        private:  int mBaseTextureID;
        private:  int mMvpConstantID;

        private:  int mNumPasses;
        private:  std::vector<Texture*>* mPassTextures;

        private:  std::string mMode;
        ...
        private:  VertexBuffer3D* mVertexBuffer;
        private:  std::vector<unsigned int>* mIndexData;
        private:  IndexBuffer3D* mIndexBuffer;

        private:  bool mCacheRequested;
        private:  QuadBatch* mCache;

        /** helper objects. */
        private:  Matrix* mProjMatrix;
        private: static  Rectangle* sBounds;
        private: static  Rectangle* sStageBounds;
        private: static  Matrix* sTransformationMatrix;
        ...
        public:          FragmentFilter(int numPasses=1, float resolution =1.0);

        /** Disposes the filter (programs, buffers, textures). */
        public: void     dispose();

        private: void     onContextCreated(Object* event);
        ...
        public: void     render(DisplayObject* object, RenderSupport* support, float parentAlpha);

        private: QuadBatch* renderPasses(DisplayObject* object, RenderSupport* support,
                                      float parentAlpha, bool intoCache   =false);

        // helper methods// final pass

        private: void     updateBuffers(Context3D* context, Rectangle* bounds);

        private: void     updatePassTextures(int width, int height, float scale);

        private: Texture* getPassTexture(int pass);
        ...
        private: void     calculateBounds(DisplayObject* object, Stage* stage,
        ...
        protected: void     activate(int pass, Context3D* context, Texture* texture);
        ...
        protected: void     deactivate(int pass, Context3D* context, Texture* texture);
        ...
        protected: Program3D* assembleAgal(std::string fragmentShader=NULL, std::string vertexShader=NULL);
        ...
        public: void         mode(std::string value);
