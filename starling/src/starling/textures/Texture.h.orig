...
namespace textures {
    class Texture
    {
        ...
        public: static Texture* fromBitmap(Bitmap* data, bool generateMipMaps   =true,
                                          bool optimizeForRenderToTexture   =false,
                                          float scale =1);

        /** Creates a texture from bitmap data. 
         *  Beware: you must not dispose 'data' if Starling should handle a lost device context. */
        public: static Texture* fromBitmapData(BitmapData* data, bool generateMipMaps   =true,
                                              bool optimizeForRenderToTexture   =false,
                                              float scale =1);
        ...
        public: static Texture* fromAtfData(ByteArray* data, float scale =1, bool useMipMaps   =true,
         ...
         */
        public: static Texture* fromColor(int width, int height, unsigned int color=0xffffffff,
                                         bool optimizeForRenderToTexture   =false,
                                         float scale =-1);
         ...
         *  @param scale:  if you omit this parameter, 'Starling.contentScaleFactor' will be used.
         */
        public: static Texture* empty(int width=64, int height=64, bool premultipliedAlpha   =false,
                                     bool optimizeForRenderToTexture   =true,
                                     float scale =-1);

        /** Creates a texture that contains a region (in pixels) of another texture. The new
         *  texture will reference the base texture; no data is duplicated. */
        public: static Texture* fromTexture(Texture* texture, Rectangle* region=NULL, Rectangle* frame=NULL);

        /** Converts texture coordinates and vertex positions of raw vertex data into the format 
         *  required for rendering. */
        public: void     adjustVertexData(VertexData* vertexData, int vertexID, int count);
