...
namespace utils {
    ...
    {
        /** Helper object. */
        private: static  std::vector<float>* sRawData;
        ...
        public: static Matrix3D* convertTo3D(Matrix* matrix, Matrix3D* resultMatrix=NULL);
        ...
        public: static Point* transformCoords(Matrix* matrix, float x, float y,
         ...
         *  </pre> 
         */
        public: static void skew(Matrix* matrix, float skewX, float skewY);

        /** Prepends a matrix to 'base' by multiplying it with another matrix. */
        public: static void prependMatrix(Matrix* base, Matrix* prep);

        /** Prepends an incremental translation to a Matrix object. */
        public: static void prependTranslation(Matrix* matrix, float tx, float ty);

        /** Prepends an incremental scale change to a Matrix object. */
        public: static void prependScale(Matrix* matrix, float sx, float sy);

        /** Prepends an incremental rotation to a Matrix object (angle in radians). */
        public: static void prependRotation(Matrix* matrix, float angle);
         ...
         *  </pre> 
         */
        public: static void prependSkew(Matrix* matrix, float skewX, float skewY);
    };
}
}
