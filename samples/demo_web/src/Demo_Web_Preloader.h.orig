...
#include "Class.h"
#include "RegExp.h"
namespace flash { namespace display { class DisplayObject; } }
#include "flash/display/MovieClip.h"
namespace flash { namespace display { class Shape; } }
...
using namespace starling::utils;
    ...
    {
        private: const std::string STARTUP_CLASS;

        private:  Shape* mProgressIndicator;
        private:  int mFrameCount;

        public:          Demo_Web_Preloader();

        private: void     onAddedToStage(Event* event);

        private: void     onEnterFrame(Event* event);

        private: Shape*   createProgressIndicator(float radius =12, int elements=8);
