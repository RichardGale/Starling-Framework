...
namespace utils {
    class VertexData
    {
        ...
        public: static const int ELEMENTS_PER_VERTEX;

        /** The offset of position data (x, y) within a vertex. */
        public: static const int POSITION_OFFSET;

        /** The offset of color data (r, g, b, a) within a vertex. */
        public: static const int COLOR_OFFSET;

        /** The offset of texture coordinates (u, v) within a vertex. */
        public: static const int TEXCOORD_OFFSET;

        private:  std::vector<float>* mRawData;
        ...
        public: void     getTexCoords(int vertexID, Point* texCoords);

        // utility functions

        /** Translate the position of a vertex by a certain offset. */
        public: void     translateVertex(int vertexID, float deltaX, float deltaY);

        /** Transforms the position of subsequent vertices by multiplication with a 
         *  transformation matrix. */
        public: void     transformVertex(int vertexID, Matrix* matrix, int numVertices=1);

        /** Sets all vertices of the object to the same color values. */
        public: void     setUniformColor(unsigned int color);
        ...
        public: void     scaleAlpha(int vertexID, float alpha, int numVertices=1);
         ...
         *  'numVertices' to '-1'. */
        public: Rectangle* getBounds(Matrix* transformationMatrix=NULL,
                                  int vertexID=0, int numVertices=-1,
        ...
        public: void     setPremultipliedAlpha(bool value, bool updateData   =true);
        ...
        public: void         numVertices(int value);

        /** The raw vertex data; not a copy! */
        public: std::vector<float>* rawData();
    };
}
}
