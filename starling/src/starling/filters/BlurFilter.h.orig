...
namespace filters {
    ...
    {
        private: const float MAX_SIGMA;

        private:  Program3D* mNormalProgram;
        private:  Program3D* mTintedProgram;

        private:  std::vector<float>* mOffsets;
        private:  std::vector<float>* mWeights;
        private:  std::vector<float>* mColor;
        ...
        /** helper object */
        private:  std::vector<float>* sTmpWeights;
         ...
         *  cost.</p>
         */
        public:          BlurFilter(float blurX =1, float blurY =1, float resolution =1);

        /** Creates a blur filter that is set up for a drop shadow effect. */
        public: static BlurFilter* createDropShadow(float distance =4.0, float angle =0.785,
                                                unsigned int color=0x0, float alpha =0.5, float blur =1.0,
                                                float resolution =0.5);

        /** Creates a blur filter that is set up for a glow effect. */
        public: static BlurFilter* createGlow(unsigned int color=0xffff00, float alpha =1.0, float blur =1.0,
                                          float resolution =0.5);
        ...
        private: Program3D* createProgram(bool tinted);
