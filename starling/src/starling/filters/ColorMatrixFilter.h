...
namespace filters {
    ...
    {
        private:  Program3D* mShaderProgram;

        private:  std::vector<float>* mUserMatrix;   // offset in range 0-255
        private:  std::vector<float>* mShaderMatrix; // offset in range 0-1, changed order

        private: static const std::vector<float>* MIN_COLOR;
        private: static const std::vector<void*> IDENTITY;
        private: static const float LUMA_R;
        private: static const float LUMA_G;
        private: static const float LUMA_B;

        /** helper objects */
        private: static  std::vector<float>* sTmpMatrix1;
        private: static  std::vector<float>* sTmpMatrix2;

        /** Creates a new ColorMatrixFilter instance with the specified matrix. 
         *  @param matrix: a vector of 20 items arranged as a 4x5 matrix.   
         */
        public:          ColorMatrixFilter(std::vector<float>* matrix=NULL);
        ...
        protected: virtual void     activate(int pass, Context3D* context, Texture* texture);
        ...
        public: void     concat(std::vector<float>* matrix);

        /** Concatenates the current matrix with another one, passing its contents directly. */
        private: void     concatValues(float m0, float m1, float m2, float m3, float m4,
        ...
        private: void     copyMatrix(std::vector<float>* from, std::vector<float>* to);
        ...
        public: std::vector<float>* matrix();
        public: void         matrix(std::vector<float>* value);
    };
}
}
