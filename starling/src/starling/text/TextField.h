...
namespace flash { namespace display { class StageQuality; } }
namespace flash { namespace geom { class Matrix; } }
namespace flash { namespace geom { class Rectangle; } }
namespace flash { namespace text { class AntiAliasType; } }
...
namespace text {
    class TextField: public DisplayObjectContainer
    {
        ...
        private: static const std::string BITMAP_FONT_DATA_NAME;

        private:  float mFontSize;
        private:  unsigned int mColor;
        private:  std::string mText;
        private:  std::string mFontName;
        private:  std::string mHAlign;
        private:  std::string mVAlign;
        private:  bool mBold;
        private:  bool mItalic;
        private:  bool mUnderline;
        private:  bool mAutoScale;
        private:  std::string mAutoSize;
        private:  bool mKerning;
        private:  std::vector<void*> mNativeFilters;
        private:  bool mRequiresRedraw;
        private:  bool mIsRenderedText;
        private:  Rectangle* mTextBounds;

        private:  DisplayObject* mHitArea;
        private:  DisplayObjectContainer* mBorder;

        private:  Image* mImage;
        private:  QuadBatch* mQuadBatch;

        // this object will be used for text rendering
        private: static  flash::text::TextField* sNativeTextField;

        /** Create a new text field with the given properties. */
        public:          TextField(int width, int height, std::string text, std::string fontName="Verdana",
         ...
         */
        protected: void     formatText(flash::text::TextField* textField, TextFormat* textFormat);

        private: void     autoScaleNativeTextField(flash::text::TextField* textField);
        ...
        /** @inheritDoc */
        public: virtual void         width(float value);

        /** @inheritDoc */
        public: virtual void         height(float value);

        /** The displayed text. */
        public: std::string  text();
        public: void         text(std::string value);

        /** The name of the font (true type or bitmap font). */
        public: std::string  fontName();
        public: void         fontName(std::string value);
        ...
        public: std::string  hAlign();
        public: void         hAlign(std::string value);

        /** The vertical alignment of the text. @default center @see starling.utils.VAlign */
        public: std::string  vAlign();
        public: void         vAlign(std::string value);
        ...
        public: std::string  autoSize();
        public: void         autoSize(std::string value);

        /** The native Flash BitmapFilters to apply to this TextField. 
         *  Only available when using standard (TrueType) fonts! */
        public: std::vector<void*> nativeFilters();
        public: void         nativeFilters(std::vector<void*> value);
        ...
        public: static std::string registerBitmapFont(BitmapFont* bitmapFont, std::string name=NULL);

        /** Unregisters the bitmap font and, optionally, disposes it. */
        public: static void unregisterBitmapFont(std::string name, bool dispose   =true);

        /** Returns a registered bitmap font (or null, if the font has not been registered). */
        public: static BitmapFont* getBitmapFont(std::string name);
        ...
        private: static std::map<std::string, void*> bitmapFonts();
    };
}
}
